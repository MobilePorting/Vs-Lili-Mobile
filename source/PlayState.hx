...
//cosos que no se que hacen pero no funca sin ellos xD
...
class PlayState extends MusicBeatState
{
	...
	//vs Lili variables del fondo
	var house:BGSprite;
	var bed:BGSprite;
	var callejon:BGSprite;
	var celos:BGSprite;
	var outside:BGSprite;
	var sala:BGSprite;
	var milk:BGSprite;
	var redvignette:BGSprite;
	var vignette:FlxSprite;
	var darkoverlay:FlxSprite;
	//Personajes y weas animadas
	var bfmadBG:BGSprite;
	var gfmadBG:BGSprite;
	var lilimadBG:BGSprite;
	var liliBG:BGSprite;
	var lulumadBG:BGSprite;
	//Misc variables
	var darkScreen:FlxSprite;
	var defaultStageZoom:Float = 0.9;
	private var task:SongInfo;
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			...
			detailsText = "Story Mode: Full lili week";
		}
		else
		{
		...
		switch (curStage)
		{
			...
			case 'bed': //Para noche de chicas
				...
				bed.setGraphicSize(Std.int(bed.width * 1.6));
				bed.updateHitbox();
				add(bed);

			case 'callejon': //Para game of robots
				callejon = new BGSprite('fondos/callejon', -700, -520, 0.9, 0.9);
				callejon.setGraphicSize(Std.int(callejon.width * 1.6));
				callejon.updateHitbox();
				add(callejon);

			case 'celos': //Para celos
				celos = new BGSprite('fondos/celos', -700, -520, 0.9, 0.9);
				celos.setGraphicSize(Std.int(celos.width * 1.6));
				celos.updateHitbox();
				add(celos);
			...
			case 'outside': //Para todas las de Lulu
				outside = new BGSprite('fondos/outside', -700, -520, 0.9, 0.9);
				outside.setGraphicSize(Std.int(outside.width * 1.6));
			...
			case 'sala': //Para el cover de deep poems
				sala = new BGSprite('fondos/sala', -700, -520, 0.9, 0.9);
				sala.setGraphicSize(Std.int(sala.width * 1.6));
				sala.updateHitbox();
				add(sala);
			case 'milk': //milk xd
				milk = new BGSprite('fondos/milkstage', -700, -520, 0.9, 0.9);
				milk.setGraphicSize(Std.int(milk.width * 1.6));
				milk.updateHitbox();
				add(milk);

			//Para cada cancion especifica
		...
		{
			case 'double-or-nothing':
				GameOverSubstate.characterName = 'bf-mad-dead';
			case 'my-history':
				GameOverSubstate.characterName = 'bf-pijama';
		...
		//Robado de DDTO xD
		vignette = new FlxSprite(0, 0).loadGraphic(Paths.image('vignette', 'lili'));
...
//		redvignette.setGraphicSize(FlxG.width, FlxG.height);
		...
		for (folder in foldersToCheck)
		{
				...
				for (file in SUtil.readDirectory(folder))
				{
					...
					{
						luaArray.push(new FunkinLua(#if MODS_ALLOWED folder + #end file));
		...
		switch (daSong)
			{
				case 'my-history' | 'little-suprise' | 'double-or-nothing': //Pantalla negra al incio de las canciones
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, value3:String) {
		switch(eventName) {
			...
			case 'Vignette':
				...
				var val2:Float = Std.parseFloat(value2);
				// Value 1 alpha
				// Value 2 velocidad de aparicion
			...
			case 'Pantalla negra':
				...
				FlxTween.tween(darkoverlay, {alpha: val1}, val2, {ease: FlxEase.linear});	
                        ...
                        case 'Camera Flash':
                                var val2:Float = Std.parseFloat(value2);
                                var val1:FlxColor = CoolUtil.colorFromString(value1);
			...
			case 'Red Vignette':
				...
				FlxTween.cancelTweensOf(redvignette);
				...
				if (val2 != 0)
					FlxTween.tween(redvignette, {alpha: val1}, val2, {ease: FlxEase.linear, onComplete: function(twn:FlxTween){}});
	...
	public function endSong():Void
	{
				...
				if (!seenCutscene)
		{
			...
			seenCutscene = true;
				...
				{
					if(!ClientPrefs.storycomplete && !ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false))
					{
